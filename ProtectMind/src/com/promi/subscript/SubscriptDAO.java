...
///해당 회원의 정보를 가져오기///
	...
	public List<Subscript> getBuyList(String id) {
		...
		try {
			conn();
			String sql = "select products_name,product_num,product_spend,buy_day,delivery,order_number from Subscript where consumer_id=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, id);
...
//배송목차만 가져오기
	...
	public List<Subscript> getDeliverList(String id) {
		...
		try {
			conn();
			String sql = "select products_name,product_num,product_spend,buy_day,delivery ,order_number,review_c"
			...
			while (rs.next()) {
				...
				sub.setDelivery(rs.getInt("delivery"));
				sub.setOrderNumber(rs.getInt("order_number"));
				sub.setReview(rs.getInt("review_c"));
				list.add(sub);
			}
...
//배송 1을 2로 바꾸기
	public int setDeliverComple(int num) {
		int result = 0;
	...
	//2->0으로만들기 구매확정
	public int setDeliverMem(int num) {
		int result = 0;

		try {
			conn();
			String sql = "update Subscript set delivery=0 where order_number=? ";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
			result = pstmt.executeUpdate();
	...
	//상품번호로 리뷰남기기
	public int review(int review , int num ) {
		int result = 0;

		try {
			conn();
			String sql = "update subscript set review_c = ? where order_number =?";
			pstmt = conn.prepareStatement(sql);
...
//상품 번호로 내용 가져오기//
	public Subscript getDeliver(int num) {
		
		Subscript sub = null;
		try {
			conn();
			String sql = "select * from Subscript where order_number=? ";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
